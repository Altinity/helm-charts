...
def wait_for_pod_count(self, namespace, expected_count, timeout=300):
    ...
    while True:
        ...
        if time.time() - start_time > timeout:
            raise TimeoutError(f"Timeout waiting for {expected_count} pods in namespace {namespace}")
        time.sleep(5)
...
def wait_for_pods_running(self, namespace, timeout=300):
    ...
    while True:
        ...
        if time.time() - start_time > timeout:
            ...
            raise TimeoutError(f"Timeout waiting for pods to be running. Pod statuses: {pod_statuses}")
