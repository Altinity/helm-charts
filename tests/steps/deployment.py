...
class HelmState:
    ...
    def verify_cluster_topology(self, namespace):
        ...
        note(f"✓ Cluster topology: {expected_replicas} replicas, {expected_shards} shards")
    ...
    def verify_pvc_access_mode(self, namespace, expected_access_mode, pvc_type='data'):
        ...
        for pvc in pvcs:
            if pvc_type in pvc.lower() and clickhouse.is_clickhouse_resource(resource_name=pvc):
                pvc_info = kubernetes.get_pvc_info(namespace=namespace, pvc_name=pvc)
                access_modes = pvc_info.get("spec", {}).get("accessModes", [])
                
                assert expected_access_mode in access_modes, \
    ...
    def verify_pod_annotations(self, namespace):
        ...
        clickhouse.verify_pod_annotations(
            namespace=namespace,
            expected_annotations=pod_annotations
        )
    ...
    def verify_log_persistence(self, namespace):
        ...
        if logs_config.get('enabled'):
            ...
            clickhouse.verify_log_persistence(
                namespace=namespace,
                expected_log_size=expected_log_size
            )
    ...
    def verify_keeper_annotations(self, namespace):
        ...
        if keeper_annotations:
            clickhouse.verify_keeper_annotations(
                namespace=namespace,
                expected_annotations=keeper_annotations
            )
    ...
    def verify_keeper_resources(self, namespace):
        ...
        if resources_config:
            ...
            # Handle requests
            if 'cpuRequestsMs' in resources_config or 'memoryRequestsMiB' in resources_config:
            ...
            # Handle limits
            if 'cpuLimitsMs' in resources_config or 'memoryLimitsMiB' in resources_config:
    ...
    def verify_all(self, namespace):
        ...
        if self.clickhouse_config.get('persistence', {}).get('enabled'):
            ...
            if self.clickhouse_config.get('persistence', {}).get('logs', {}).get('enabled'):
